...
export function FileDialog<TFieldValues extends FieldValues>({
  ...
  const onDrop = React.useCallback(
    ...
    [maxSize, maxFiles, setFiles]
  )

  // Register files to react-hook-form
  React.useEffect(() => {
    if (Number(files?.length) > maxFiles) {
      toast.error(
        `You can only upload ${maxFiles} ${maxFiles === 1 ? "file" : "files"}`
      )
      setFiles((prev) => (prev ?? []).splice((prev ?? []).length - 1, 1))
      return
    }
  ...
  React.useEffect(() => {
    return () => {
      if (!files || !revokePreviewOnUnmount) return
